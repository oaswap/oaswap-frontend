...
import { CardBody, Flex, Text, CardRibbon } from '@oaswap/uikit'
...
const PoolCard: React.FC<{ pool: DeserializedPool; account: string }> = ({ pool, account }) => {
  ...
  const isCakePool = earningToken.symbol === 'OAS' && stakingToken.symbol === 'OAS'
    ...
    >
      <PoolCardHeader isStaking={accountHasStakedBalance} isFinished={isFinished && sousId !== 0}>
        <PoolCardHeaderTitle
          ...
          subTitle={isCakePool ? t('Earn OAS, stake OAS') : t('Stake %symbol%', { symbol: stakingToken.symbol })}
        />
