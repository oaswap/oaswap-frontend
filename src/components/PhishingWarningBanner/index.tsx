...
import { Text, Flex, Box, CloseIcon, IconButton, useMatchBreakpoints } from '@oaswap/uikit'
...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://oaswap.finance - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/oaswap.finance)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://oaswap.finance'}
          color={text === 'https://oaswap.finance' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
      ...
      {isMobile || isMd ? (
        ...
        <>
          <InnerContainer>
            ...
            <SpeechBubble style={{ justifyContent: 'center' }}>{warningTextComponent}</SpeechBubble>
          </InnerContainer>
          <IconButton onClick={hideBanner} variant="text">
            <CloseIcon color="#FFFFFF" />
          </IconButton>
